...
if (isset($db->hp)) 
	$q = mysql_query("SELECT i,name,level FROM ".$db->hpdb["homepage"].".ranking_cache ORDER BY i ASC LIMIT ".$plugin_conf["count"],$db->hp);
else
	$q = mysql_query("SELECT i,name,level FROM ".$db->gamedb["homepage"].".ranking_cache ORDER BY i ASC LIMIT ".$plugin_conf["count"],$db->game);
$b = "";
...
$content = array(
	...
	"middle" => array(
		"text" => '<table class="sbranking"><tr><td>#</td><td>'.$lang["misc"]["user"].'</td><td>'.$lang["misc"]["level"].'</td>'.$b.'</table><a class="sb-link" href="'.$urlmap["ranking"].'">'.$lang["misc"]["ranking"].'</a>'
	)
);
unset($plugin_conf);
?>
