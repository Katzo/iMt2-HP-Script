...
if ($_POST["v"]){// Check if he has voted
	...
	if (isset($db->hp))
		...
		die(json_encode(array("ok" => $lang["vote"]["success"],"btn" => $lang["misc"]["vote"])));
	}
	...
	die(json_encode(array("btn" => $lang["misc"]["check"],"url" => "http://www.topliste.top-pserver.com/In/".$plugin_conf["id"]."-.htm")));
}
