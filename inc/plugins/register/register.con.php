		...
		$content = array(
			...
			"middle" => array(
				"text" => ((isset($regerror))?'<font class="error">'.$regerror.'</font>':'').'<form method="POST"><table style="width:90%;">
							...
							<tr><td>'.$lang["misc"]["security_answer"].'</td><td><input class="bar" type="text" placeholder="'.$lang["misc"]["security_answer"].'" name="security_answer" id="security_answer" onblur="javascript:regcheck(\'security_answer\');" '.((isset($_POST["security_answer"]))?'value="'.$_POST["security_answer"].'" ':'').'/></td><td id="security_answerres"></td></tr>
