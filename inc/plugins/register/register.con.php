		...
		$content = array(
			...
			"middle" => array(
				"text" => ((isset($regerror))?'<font class="error">'.$regerror.'</font>':'').'<form method="POST"><table style="width:90%;">
							...
							<tr><td>'.$lang["misc"]["security_question"].'</td><td><input class="bar" type="text" placeholder="'.$lang["misc"]["security_question"].'" name="security_question" id="security_question" onblur="javascript:regcheck(\'security_question\');" '.((isset($_POST["code"]))?'value="'.$_POST["code"].'" ':'').'/></td><td id="security_questionres"></td></tr>
							<tr><td>'.$lang["misc"]["security_answer"].'</td><td><input class="bar" type="text" placeholder="'.$lang["misc"]["security_answer"].'" name="security_answer" id="security_answer" onblur="javascript:regcheck(\'security_answer\');" '.((isset($_POST["code"]))?'value="'.$_POST["code"].'" ':'').'/></td><td id="security_answerres"></td></tr>
