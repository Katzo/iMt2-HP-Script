...
$q = mysql_query("SELECT id,name,level,job FROM ".$config["db"]["game"]["db"]["player"].".player LEFT JOIN ".$config["db"]["game"]["db"]["common"].".gmlist ON gmlist.mName = player.name WHERE gmlist.mName is null AND account.status = 'OK' ORDER BY level DESC,exp DESC",$game);
$i=0; // General
$wi=0; // Warrior
