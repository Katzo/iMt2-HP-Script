...
if (!$info) die(json_encode(array("error" => $lang->get("itemshop_itemerror"))));
if ($_SESSION["coins"] < $info->price) die(json_encode(array("error" => $lang->get("itemshop_priceerror"))));
$realcoins = mysql_fetch_object(mysql_query("SELECT ".$settings->get("coin")." as coins FROM ".$db->gamedb["account"].".account WHERE id='".$_SESSION["id"]."' LIMIT 1",$db->game))->coins;
//if ($realcoins != $_SESSION["coins"]) dostuff(); <- Comming later. Probably. Maybe.
if ($realcoins < $info->price) die(json_encode(array("error" =>$lang->get("itemshop_priceerror"),"coins" => $_SESSION["coins"])));
if (isset($db->hp))
...
mysql_query('UPDATE '.$db->gamedb["account"].'.account SET '.$settings->get("coin").' = "'.($realcoins-$info->price).'" WHERE id="'.$_SESSION["id"].'" LIMIT 1',$db->game);
...
if (isset($_POST["ingame"])) // Yay! When user reaches this he successfully bought something! money! $_$
	die(json_encode(array("ok" => $lang->get("itemshop_success").'&nsbp;&nsbp;<a style="display:inline-block;" onclick="javascript:buy($(this).parent(),'.$_POST["id"].');return false;" class="more">'.$lang->get("buy_again").'</a>'))); 
else
	die(json_encode(array("ok" => $lang->get("itemshop_success").'&nsbp;&nsbp;<a style="display:inline-block;" onclick="javascript:buy($(this).parent(),'.$_POST["id"].');return false;" class="more">'.$lang->get("buy_again").'</a>',"coins" => $_SESSION["coins"])));
?>
