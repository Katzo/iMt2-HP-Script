...
$content=array(
 ...
 if (isset($db->hp))
 	$q = mysql_query('SELECT id,name,desc,img,price FROM '.$db->hpdb["homepage"].'.itemshop_package ORDER BY added DESC LIMIT '.$plugin_conf["newest_count"],$db->hp);
 else
 	$q = mysql_query('SELECT id,name,desc,img,price FROM '.$db->gamedb["homepage"].'.itemshop_package ORDER BY added DESC LIMIT '.$plugin_conf["newest_count"],$db->game);
