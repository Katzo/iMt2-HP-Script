...
while ($res = mysql_fetch_object($q)) {
	$content[]=array(
		...
		"footer" => array(
			"plain" => $lang["itemshop"]["price"].': <b>'.$res->price.'</b> '.($res->price==1?$lang["misc"]["coin"]:$lang["misc"]["coins"]).'<input type="button" '.(isset($_SESSION["user"])&&!empty($_SESSION["user"])?'onclick="javascript:buy(this,'.$res->id.');"':'disabled="disabled"').' value="'.$lang["itemshop"]["buy"].'" class="btn right" />'
		)
	);
}
